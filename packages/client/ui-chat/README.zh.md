...
较长的用户与 steering 消息默认显示前 100 个词，并通过本地化的「显示更多」控件展开完整消息；复制操作始终使用原始全文。已发送的用户消息提供行内编辑操作，可将消息气泡切换为自适应高度的文本框以修改提示词、取消编辑，或从该轮前分支并使用新输入重新开始对话。
